...
class CondVar {
 public:
  ...
  void TimedWait(Mutex *mu, uint64_t micros) {
    condvar_wait_timed(&cv_, &mu->mu_, micros);
  }
...
class Semaphore {
 public:
  // initializes a semaphore with the provided count.
  Semaphore(int count) { semaphore_init(&sem_, count); }

  // Acquire a semaphore.
  void Wait() { semaphore_wait(&sem_); }

  // Try to acquire a semaphore.
  bool TryWait() { return semaphore_try_wait(&sem_); }

  // Release a semaphore.
  void Post() { semaphore_post(&sem_); }
