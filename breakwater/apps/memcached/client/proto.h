...
static_assert(sizeof(MemcachedReqHdr) == 32);

struct MemcachedRespHdr {
  uint8_t magic;
  uint8_t opcode;
  uint16_t key_length;
  uint8_t extra_length;
...
static_assert(sizeof(MemcachedRespHdr) == 24);



void hton(struct MemcachedReqHdr *hdr) {
...
void ntoh(struct MemcachedRespHdr *hdr) {
...
ssize_t ConstructMemcachedSetReq(
  ...
  if ((unsigned int)_buflen < sizeof(struct MemcachedReqHdr) + body_len)
    return 0;

  /* header */
  hdr = reinterpret_cast<struct MemcachedReqHdr *>(_buf);
