...
/* Dummy per-core response buffer */
char **dummy_resp_buf;
/* Memory semaphore for long requests */
MemSemaphoreHandle *msem = NULL;
...
static void memcached_handler(struct srpc_ctx *ctx)
{
    ...
    req_hdr = (protocol_binary_request_header *)ctx->req_buf;

    /* Extract the opaque ID */
    if (settings.send_empty_responses) {
        opaque = req_hdr->request.opaque;
    }

    /* Intialize the connection with the RPC context */
    c->rbuf = ctx->req_buf;
    c->rcurr = c->rbuf;
    c->rbytes = ctx->req_len;
    if (settings.send_empty_responses) {
        c->out = dummy_resp_buf[core_id];
    } else {
        c->out = ctx->resp_buf;
    }

    if ((req_hdr->request.opcode != PROTOCOL_BINARY_CMD_GET) ||
        req_hdr->request.is_skey) {
        /* For any non-GET request, directly call into core memcached */
        /* For GETS on small keys, directly call into core memcached */
        drive_machine(c);
    } else {
        /* For GETs on large keys, go through the memory semaphore */
        if (!settings.use_msem) {
            drive_machine(c);
        } else {
            if (MemSemaphore_TryWait(msem)) {
                drive_machine(c);
                MemSemaphore_Post(msem);
            } else {
                ctx->drop = true;
                return;
            }
    ...
    if (settings.send_empty_responses) {
        /* Only extract the header */
        memcpy(ctx->resp_buf, dummy_resp_buf[core_id],
               sizeof(protocol_binary_response_header));
        ctx->resp_len = sizeof(protocol_binary_response_header);

        /* Initialize an empty body in the header */
        protocol_binary_response_header *resp_hdr = \
            (protocol_binary_response_header *)ctx->resp_buf;
        resp_hdr->response.keylen = 0;
        resp_hdr->response.extlen = 0;
        resp_hdr->response.bodylen = 0;
        /* There should technically be no need to do this.
         * But looks like memcached is not setting opaque properly
         * when the requested key's values are large.
         *
         * The following asserts trip at this point. It indicates a
         * bug in memcached or caladan?
         *  BUG_ON(opaque != hdr->response.opaque);
         *  BUG_ON(core_id != get_current_affinity());
         *
         * It looks like caladan is scheduling this userthread on
         * different cores. We are not run-to-completing, especially
         * for long requests.
         */
        resp_hdr->response.opaque = opaque;
    } else {
        /* We should technically not use 'c' here, as it is deallocated
         * in drive machine. But lets not change things that do not fail.
         */
    ...
    while (-1 != (c = getopt(argc, argv, shortopts))) {
       ...
       case 'X' :
        ...
        case 'o': /* It's sub-opts time! */
            ...
            case SKEY_COUNT:
                if (subopts_value == NULL) {
                    fprintf(stderr, "Missing skey_count argument\n");
                    exit(1);
                }
            ...
            case LKEY_COUNT:
                if (subopts_value == NULL) {
                    fprintf(stderr, "Missing lkey_count argument\n");
                    exit(1);
                }
                settings.lkey_count = atoi(subopts_value);
                break;
            case SEND_EMPTY_RESPONSES:
                settings.send_empty_responses = true;
                break;
            case USE_MSEM:
                settings.use_msem = true;
                break;
            default:
...
static void rpc_stat_worker(void *arg) {
    ...
    while(true) {
        ret = tcp_read_full(c, &magic, sizeof(magic));
        if (ret <= 0) goto done;
        ...
        sstat_raw.energy_consumed = runtime_glob_energy_consumed();
        ...
        ret = tcp_write_full(c, &sstat_raw, sizeof(sstat_raw));
        assert(ret > 0);
    }
done:
    tcp_close(c);
...
static ssize_t construct_set_req(
  ...
  if ((unsigned int)_buflen < sizeof(protocol_binary_request_header) + body_len)
    return 0;

  /* header */
  hdr = (protocol_binary_request_header *)_buf;
  hdr->request.magic = 0x80;
  hdr->request.opcode = 0x01;
  hdr->request.keylen = _key_len;
  hdr->request.extlen = 8;
  hdr->request.datatype = 0;
  hdr->request.reserved = 0;
  hdr->request.bodylen = body_len;
  hdr->request.opaque = _id;
  hdr->request.cas = 0;

  /* Convert the header to the network byte order */
  hdr->request.keylen = hton16(hdr->request.keylen);
  hdr->request.reserved = hton16(hdr->request.reserved);
  hdr->request.bodylen = hton32(hdr->request.bodylen);
  hdr->request.opaque = hton32(hdr->request.opaque);
  hdr->request.cas = hton64(hdr->request.cas);

  buf += sizeof(protocol_binary_request_header);
...
static void prepopulate_bimod_keys() {
    ...
    /* Exit if there is nothing to populate */
    if (!settings.skey_count && !settings.lkey_count) {
        return;
    }
    ...
    for (int i = 0; i < settings.lkey_count; ++i) {
...
static void create_dummy_response_buffers() {

    int dummy_resp_buf_size;
    int num_cores;

    dummy_resp_buf_size = sizeof(protocol_binary_response_header) + 8 + \
        KEY_MAX_LENGTH + settings.item_size_max;

    num_cores = runtime_max_cores();

    dummy_resp_buf = malloc(num_cores * sizeof(char *));
    BUG_ON(!dummy_resp_buf);
    for (int i = 0; i < num_cores; ++i) {
        dummy_resp_buf[i] = malloc(dummy_resp_buf_size);
        BUG_ON(!dummy_resp_buf[i]);
    }
}
...
#define NB_TIMES (10000)

static ssize_t construct_get_req(
    char* _buf, int _buflen, uint32_t _id, const char *_key, uint16_t _key_len) {
  char *buf = _buf;
  protocol_binary_request_header *hdr;
...
static void profile_service_times() {

    char key[KEY_MAX_LENGTH];
    int key_len;
    char req_buf[2048];
    int req_buf_size = 2048;
    int req_len;
    ...
    for (int i = 0; i < NB_TIMES; ++i) {
        ...
        key_len = snprintf(key, KEY_MAX_LENGTH, "skey-%d", i % settings.skey_count);

        /* Construct the request buffer */
        req_len = construct_get_req(req_buf, req_buf_size, 0,
                                    key, key_len);
        BUG_ON(!req_len);
        ...
        c->rbuf = (char*)&req_buf[0];
        c->rcurr = c->rbuf;
        c->rbytes = req_len;
        c->out = dummy_resp_buf[0];
    ...
    for (int i = 0; i < NB_TIMES; ++i) {
        ...
        key_len = snprintf(key, KEY_MAX_LENGTH, "lkey-%d", i % settings.lkey_count);
...
static void memcached_main(void *arg)
{
   ...
   if (start_logger_thread() != 0) {
    ...
    /* Create dummy response buffers if empty response mode is requested */
    if (settings.send_empty_responses) {
        create_dummy_response_buffers();
    }

#ifdef PROFILE_ST
    /* Profile the service times */
    profile_service_times();
#endif
...
int main(int argc, char **argv) {
    ...
    if (strncmp(ocalg, "breakwater", 10) == 0) {
        crpc_ops = &cbw_ops;
        srpc_ops = &sbw_ops;
    } else if (strncmp(ocalg, "protego", 7) == 0) {
        crpc_ops = &cbw_ops;
        srpc_ops = &sbw2_ops;
    } else if (strncmp(ocalg, "pcc", 3) == 0) {
        crpc_ops = &cpcc_ops;
        srpc_ops = &spcc_ops;
