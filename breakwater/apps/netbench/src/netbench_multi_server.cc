...
// Percentage of cpu-bound requests generated by the client
...
std::vector<work_unit> GenerateWork(Arrival a, double cur_us,
  ...
  while (true) {
    ...
    if ((rand() % 100) < cpu_bound_req_pcnt) {
        is_cpu_bound_req = true;
    ...
    wu.start_us = cur_us;
    wu.duration_us = 0;
    wu.is_cpu_bound_req = is_cpu_bound_req;
...
std::vector<work_unit> ClientWorker(
  ...
  for (unsigned int i = 0; i < wsize; ++i) {
    ...
    // Send an RPC request.
    p.is_cpu_bound_req = w[i].is_cpu_bound_req;
  ...
  for (int i = 0; i < threads; ++i) {
    ...
    for (int i = 0; i < num_servers; ++i) {
        per_server_cpu_bound_req_success[i] = std::count_if(v.begin(), v.end(), [&, i](const work_unit &s) {
                return s.success && (s.served_by == (uint64_t)i) && s.is_cpu_bound_req;
            });
...
void print_client_usage() {
  std::cerr << "usage: [alg] [cfg_file] client [nclients] "
      << "[cpu_bound_work_itr] [mem_bound_work_itr] "
      << "[cpu_bound_req_pcnt] [slo] [nagents] "
      ...
      << "\tcpu_bound_work_itr: CPU-bound workload iterations (only for 'hetero')\n"
      << "\tmem_bound_work_itr: Memory-bound workload iterations (only for 'hetero')\n"
      << "\tcpu_bound_req_pcnt: CPU-bound request percentage (only for 'hetero')\n"
...
int main(int argc, char *argv[]) {
  ...
  cpu_bound_req_pcnt = std::stoi(argv[7], nullptr, 0);
