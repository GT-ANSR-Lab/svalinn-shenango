...
uint64_t kExperimentTime = 8000000;
// RTT
...
std::vector<std::vector<std::unique_ptr<rpc::RpcClient>>> clients;
...
struct cstat_raw {
  ...
  double per_server_lock_bound_req_rps[16];
  double goodput;
  ...
  double per_server_lock_bound_req_goodput[16];
...
struct work_unit {
  ...
  uint64_t work_us;
  uint64_t served_by;
  uint64_t timings[16];              // One for each server replica
  uint64_t failure_notify_delay[16]; // One for each server replica
  uint64_t sent_to;
};
 ...
 public:
  ...
  NetBarrier(int npeers) {
    ...
    for (int i = 0; i < npeers; i++) {
      ...
      BUG_ON(c->WriteFull(&kExperimentTime, sizeof(kExperimentTime)) <= 0);
  ...
  NetBarrier(netaddr leader) {
    ...
    BUG_ON(c->ReadFull(&kExperimentTime, sizeof(kExperimentTime)) <= 0);
    ...
    if (is_leader_) {
      for (auto &c : conns) {
        ...
        for (int i = 0; i < num_servers; ++i) {
            csr->per_server_cpu_bound_req_rps[i] += rem_csr.per_server_cpu_bound_req_rps[i];
            csr->per_server_mem_bound_req_rps[i] += rem_csr.per_server_mem_bound_req_rps[i];
            csr->per_server_lock_bound_req_rps[i] += rem_csr.per_server_lock_bound_req_rps[i];
        }
        ...
        for (int i = 0; i < num_servers; ++i) {
            ...
            csr->per_server_lock_bound_req_goodput[i] += rem_csr.per_server_lock_bound_req_goodput[i];
        }
        csr->min_percli_tput =
...
sstat_raw ReadRPCSStat(int server_idx) {
  std::unique_ptr<rt::TcpConn> c(
      rt::TcpConn::Dial({0, 0}, {raddr[server_idx].ip, kRPCSStatPort}));
...
shstat_raw ReadShenangoStat(int server_idx) {
  char *buf_;
  std::string buf;
  std::map<std::string, uint64_t> smap;
  std::unique_ptr<rt::TcpConn> c(
      rt::TcpConn::Dial({0, 0}, {raddr[server_idx].ip, kShenangoStatPort}));
...
std::vector<work_unit> GenerateWork(Arrival a, double cur_us,
  ...
  while (true) {
    ...
    wu.start_us = cur_us;
    wu.duration_us = 0;
    wu.req_type = req_type;
    wu.work_itr = work_itr;
    wu.hash = rand();
    wu.success = false;
    wu.is_monster = is_monster;
    for (int i = 0; i < 16; ++i) {
        wu.timings[i] = 0;
        wu.failure_notify_delay[i] = 0;
    }
    w.push_back(wu);
  }
...
std::vector<work_unit> ClientWorker(
    ...
    rt::WaitGroup *starter, rt::WaitGroup *starter2,
    std::function<std::vector<work_unit>()> wf, int thread_num) {
  std::vector<work_unit> w(wf());

  drop_arg darg;
  darg.w = (void *)w.data();
  darg.clients = (void *)clients.data();
  darg.num_servers = num_servers;
  darg.thread_num = thread_num;
  ...
  for (int i = 0; i < num_servers; ++i) {
    ths.emplace_back(rt::Thread([&, i] {
      ...
      while (true) {
        ssize_t ret = conns[i]->Recv(&rp, sizeof(rp), 0, (void *)&darg);
  ...
  for (unsigned int i = 0; i < wsize; ++i) {
    ...
    ssize_t ret = conns[0]->Send(&p, sizeof(p), w[i].hash, (void *)&darg);
...
void ClientLocalDropHandler(struct crpc_ctx *c) {
  ...
  drop_arg *darg = reinterpret_cast<drop_arg *>(c->arg);
...
std::vector<work_unit> RunExperiment(
  ...
  sstat_raw s1[16], s2[16];
  shstat_raw sh1[16], sh2[16];

  // Create one RPC client connection with every server for every thread
  ...
  for (int i = 0; i < threads; ++i) {
      for (int j = 0; j < num_servers; ++j) {
          ...
          clients[i].emplace_back(std::move(outc));
      }
  }
  ...
  th.emplace_back(rt::Thread([&] {
    auto v = ClientWorker(clients[0], &starter, &starter2, wf2, 0);
  ...
  for (int i = 1; i < threads; ++i) {
    th.emplace_back(rt::Thread([&, i] {
      srand(time(NULL) * (i+1));
      auto v = ClientWorker(clients[i], &starter, &starter2, wf, i);
      samples[i].reset(new std::vector<work_unit>(std::move(v)));
    }));
  }
  ...
  if (csr) {
    for (int i = 0; i < threads; ++i) {
      for (int j = 0; j < num_servers; ++j) {
        csr->ecredit_rx += clients[i][j]->StatEcreditRx();
        csr->cupdate_tx += clients[i][j]->StatCupdateTx();
        csr->resp_rx += clients[i][j]->StatRespRx();
        csr->req_tx += clients[i][j]->StatReqTx();
        csr->credit_expired += clients[i][j]->StatCreditExpired();
        clients[i][j]->Close();
      }
    }
  }
  ...
  uint64_t per_server_lock_bound_req_good_resps[16] = {0};
  uint64_t resps = 0;
  uint64_t cpu_bound_req_resps = 0;
  uint64_t mem_bound_req_resps = 0;
  uint64_t lock_bound_req_resps = 0;
  uint64_t per_server_cpu_bound_req_resps[16] = {0};
  uint64_t per_server_mem_bound_req_resps[16] = {0};
  uint64_t per_server_lock_bound_req_resps[16] = {0};
  uint64_t offered = 0;
  ...
  for (int i = 0; i < threads; ++i) {
    ...
    int lock_bound_req_slo_success;
    int per_server_cpu_bound_req_slo_success[16];
    int per_server_mem_bound_req_slo_success[16];
    int per_server_lock_bound_req_slo_success[16];
    int resp_success;
    int cpu_bound_req_success;
    int mem_bound_req_success;
    int lock_bound_req_success;
    int per_server_cpu_bound_req_success[16];
    int per_server_mem_bound_req_success[16];
    int per_server_lock_bound_req_success[16];
    ...
    for (int i = 0; i < num_servers; ++i) {
        per_server_cpu_bound_req_success[i] = std::count_if(v.begin(), v.end(), [&, i](const work_unit &s) {
                return s.success && (s.served_by == (uint64_t)i) && s.req_type == CPU_BOUND;
            });
        per_server_mem_bound_req_success[i] = std::count_if(v.begin(), v.end(), [&, i](const work_unit &s) {
                return s.success && (s.served_by == (uint64_t)i) && s.req_type == MEM_BOUND;
            });
        per_server_lock_bound_req_success[i] = std::count_if(v.begin(), v.end(), [&, i](const work_unit &s) {
                return s.success && (s.served_by == (uint64_t)i) && s.req_type == LOCK_BOUND;
            });
    }
    ...
    cpu_bound_req_slo_success = std::count_if(v.begin(), v.end(), [](const work_unit &s) {
      return s.success && s.req_type == CPU_BOUND && s.duration_us < slo;
    });
    mem_bound_req_slo_success = std::count_if(v.begin(), v.end(), [](const work_unit &s) {
      return s.success && s.req_type == MEM_BOUND && s.duration_us < slo;
    });
    lock_bound_req_slo_success = std::count_if(v.begin(), v.end(), [](const work_unit &s) {
      return s.success && s.req_type == LOCK_BOUND && s.duration_us < slo;
    });
    for (int i = 0; i < num_servers; ++i) {
        per_server_cpu_bound_req_slo_success[i] = std::count_if(v.begin(), v.end(), [&, i](const work_unit &s) {
                return s.success && (s.served_by == (uint64_t)i) && s.req_type == CPU_BOUND && s.duration_us < slo;
            });
        per_server_mem_bound_req_slo_success[i] = std::count_if(v.begin(), v.end(), [&, i](const work_unit &s) {
                return s.success && (s.served_by == (uint64_t)i) && s.req_type == MEM_BOUND && s.duration_us < slo;
            });
        per_server_lock_bound_req_slo_success[i] = std::count_if(v.begin(), v.end(), [&, i](const work_unit &s) {
                return s.success && (s.served_by == (uint64_t)i) && s.req_type == LOCK_BOUND && s.duration_us < slo;
            });
    }
    ...
    for (int i = 0; i < num_servers; ++i) {
        per_server_cpu_bound_req_resps[i] += per_server_cpu_bound_req_success[i];
        per_server_mem_bound_req_resps[i] += per_server_mem_bound_req_success[i];
        per_server_lock_bound_req_resps[i] += per_server_lock_bound_req_success[i];
    }
    ...
    lock_bound_req_good_resps += lock_bound_req_slo_success;
    for (int i = 0; i < num_servers; ++i) {
        ...
        per_server_lock_bound_req_good_resps[i] += per_server_lock_bound_req_slo_success[i];
    }
    if (i == 0) {
  ...
  if (csr) {
    ...
    for (int i = 0; i < num_servers; ++i) {
        csr->per_server_cpu_bound_req_rps[i] = static_cast<double>(per_server_cpu_bound_req_resps[i]) / elapsed_ * 1000000;
        csr->per_server_mem_bound_req_rps[i] = static_cast<double>(per_server_mem_bound_req_resps[i]) / elapsed_ * 1000000;
        csr->per_server_lock_bound_req_rps[i] = static_cast<double>(per_server_lock_bound_req_resps[i]) / elapsed_ * 1000000;
    }
    ...
    for (int i = 0; i < num_servers; ++i) {
        ...
        csr->per_server_lock_bound_req_goodput[i] = static_cast<double>(per_server_lock_bound_req_good_resps[i]) / elapsed_ * 1000000;
    }
  ...
  if ((!b || b->IsLeader()) && ss) {
      for (int i = 0; i < num_servers; ++i) {
          uint64_t total = s2[i].total - s1[i].total;
          ...
          uint64_t req_rx_pkts = s2[i].req_rx - s1[i].req_rx;
          uint64_t req_drop_pkts = s2[i].req_dropped - s1[i].req_dropped;
          uint64_t resp_tx_pkts = s2[i].resp_tx - s1[i].resp_tx;
          ss[i].cupdate_rx_pps = static_cast<double>(cupdate_rx_pkts) / elapsed_ * 1000000;
          ss[i].ecredit_tx_pps = static_cast<double>(ecredit_tx_pkts) / elapsed_ * 1000000;
          ss[i].credit_tx_cps = static_cast<double>(credit_tx) / elapsed_ * 1000000;
          ss[i].req_rx_pps = static_cast<double>(req_rx_pkts) / elapsed_ * 1000000;
          ss[i].req_drop_rate =
              static_cast<double>(req_drop_pkts) / static_cast<double>(req_rx_pkts);
          ss[i].resp_tx_pps = static_cast<double>(resp_tx_pkts) / elapsed_ * 1000000;

          uint64_t rx_pkts = sh2[i].rx_pkts - sh1[i].rx_pkts;
          uint64_t tx_pkts = sh2[i].tx_pkts - sh1[i].tx_pkts;
          uint64_t rx_bytes = sh2[i].rx_bytes - sh1[i].rx_bytes;
          uint64_t tx_bytes = sh2[i].tx_bytes - sh1[i].tx_bytes;
          uint64_t drops = sh2[i].drops - sh1[i].drops;
          uint64_t rx_tcp_ooo = sh2[i].rx_tcp_ooo - sh1[i].rx_tcp_ooo;
          ss[i].rx_pps = static_cast<double>(rx_pkts) / elapsed_ * 1000000;
          ss[i].tx_pps = static_cast<double>(tx_pkts) / elapsed_ * 1000000;
          ss[i].rx_bps = static_cast<double>(rx_bytes) / elapsed_ * 8000000;
          ss[i].tx_bps = static_cast<double>(tx_bytes) / elapsed_ * 8000000;
          ss[i].rx_drops_pps = static_cast<double>(drops) / elapsed_ * 1000000;
          ss[i].rx_ooo_pps = static_cast<double>(rx_tcp_ooo) / elapsed_ * 1000000;
      }
  }
...
void PrintHeader(std::ostream &os) {
  ...
  for (int i = 0; i < num_servers; ++i) {
      os << "server" << i << ":cpu_bound_req_throughput,"
         ...
         << "server" << i << ":lock_bound_req_throughput,"
         << "server" << i << ":cpu_bound_req_goodput,"
         << "server" << i << ":mem_bound_req_goodput,"
         << "server" << i << ":lock_bound_req_goodput,"
         << "server" << i << ":cpu,"
         << "server" << i << ":membw,"
         << "server" << i << ":power,"
         << "server" << i << ":rx_pps,"
         ...
         << "server" << i << ":rx_bps,"
         << "server" << i << ":tx_bps,"
         << "server" << i << ":rx_drops_pps,"
         << "server" << i << ":rx_ooo_pps,"
         << "server" << i << ":cupdate_rx_pps,"
         << "server" << i << ":ecredit_tx_pps,"
         << "server" << i << ":credit_tx_cps,"
         << "server" << i << ":req_rx_pps,"
         << "server" << i << ":req_drop_rate,"
         << "server" << i << ":resp_tx_pps,"
         << "server" << i << ":reject_mean,"
         << "server" << i << ":reject_min,"
         << "server" << i << ":reject_p50,"
         << "server" << i << ":reject_p99,";
  }
...
void PrintStatResults(std::vector<work_unit> w, struct cstat *cs,
  ...
  // Find the per-server reject delay distribution
  uint64_t per_server_reject_min[16] = {0};
  uint64_t per_server_reject_p50[16] = {0};
  double per_server_reject_mean[16] = {0.0};
  uint64_t per_server_reject_p99[16] = {0};

  for (int i = 0; i < num_servers; ++i) {
      double sum;
      std::vector<work_unit> per_server_rejected;

      std::copy_if(w.begin(), w.end(), std::back_inserter(per_server_rejected), [&, i](work_unit &s) {
              return (s.timings[i] != 0) && (s.failure_notify_delay[i] != 0);
          });
      if (per_server_rejected.size() <= 0) {
          continue;
      }

      std::sort(per_server_rejected.begin(), per_server_rejected.end(),
                [&, i](const work_unit &s1, const work_unit &s2) {
                    return s1.failure_notify_delay[i] < s2.failure_notify_delay[i];
                });
      sum = std::accumulate(per_server_rejected.begin(), per_server_rejected.end(), 0.0,
                            [&, i](double s, const work_unit &c) {
                                return s + c.failure_notify_delay[i];
                            });
      per_server_reject_min[i] = per_server_rejected[0].failure_notify_delay[i];
      per_server_reject_mean[i] = static_cast<double>(sum) / per_server_rejected.size();
      per_server_reject_p50[i] = per_server_rejected[(per_server_rejected.size() - 1) * 0.5].failure_notify_delay[i];
      per_server_reject_p99[i] = per_server_rejected[(per_server_rejected.size() - 1) * 0.99].failure_notify_delay[i];
  }
  ...
  // Get the p99 times for CPU-bound, Memory-bound, and Lock-bound requests
  ...
  if (do_load_shift) {
    ...
    all_tasks_file << "start_us,req_type,work_us,duration_us,tsc,server_queue,server_time" << std::endl;
  ...
  for (int i = 0; i < num_servers; ++i) {
      std::cout << cs->per_server_cpu_bound_req_rps[i] << ","
                ...
                << ss[i].req_drop_rate << ","
                << ss[i].resp_tx_pps << ","
                << per_server_reject_mean[i] << ","
                << per_server_reject_min[i] << ","
                << per_server_reject_p50[i] << ","
                << per_server_reject_p99[i] << ",";
  }
...
void LoadShiftExperiment(int threads) {
  ...
  std::vector<work_unit> w = RunExperiment(threads, &csr, &ss[0], &elapsed,[=] {
    std::mt19937 rg(rand());
  ...
  cs.rps = csr.rps;
  cs.cpu_bound_req_rps = csr.cpu_bound_req_rps;
  cs.mem_bound_req_rps = csr.mem_bound_req_rps;
  cs.lock_bound_req_rps = csr.lock_bound_req_rps;
  for (int i = 0; i < num_servers; ++i) {
      cs.per_server_cpu_bound_req_rps[i] = csr.per_server_cpu_bound_req_rps[i];
      cs.per_server_mem_bound_req_rps[i] = csr.per_server_mem_bound_req_rps[i];
      cs.per_server_lock_bound_req_rps[i] = csr.per_server_lock_bound_req_rps[i];
  }
  ...
  cs.lock_bound_req_goodput = csr.lock_bound_req_goodput;
  for (int i = 0; i < num_servers; ++i) {
      ...
      cs.per_server_lock_bound_req_goodput[i] = csr.per_server_lock_bound_req_goodput[i];
  }
...
void print_client_usage() {
  std::cerr << "usage: [alg] [cfg_file] client [nclients] "
      ...
      << "[req_mix] [slo] [nagents] "
      << "[offered_load] [load_shift] [test_duration_us] "
      << "[server_ip #1] [nconn #1] ...\n"
      << "\talg: overload control algorithms (breakwater/seda/dagor)\n"
      << "\tcfg_file: Shenango configuration file\n"
      ...
      << "\tload_shift: load_shift or no_load_shift\n"
      << "\ttest_duration_us: experiment duration in microseconds\n"
      << "\tserver_ip: server IP address\n"
      << "\tnconn: the number of parallel connection to the server"
      << std::endl;
}
...
int main(int argc, char *argv[]) {
  ...
  for (int j = 0; j < NUM_REQUEST_TYPES; ++j) {
	  req_mix_sum += req_mix[j];
  }
  if (req_mix_sum != 100) {
	  printf("incorrect req_mix\n");;
	  return -EINVAL;
  ...
  kExperimentTime = std::stoull(argv[13], nullptr, 0);
  ...
  for(i = 0; i < num_servers; ++i) {
    ...
    ret = StringToAddr(argv[14+2*i], &raddr[i].ip);
    if (ret) {
      std::cerr << "[Error] Cannot parse server IP:" << argv[14+2*i]
	        << std::endl;
      return -EINVAL;
    }
    raddr[i].port = kNetbenchPort;

    nconn_ = std::stoi(argv[15+2*i], nullptr, 0);
    if (nconn_ > 16) {
