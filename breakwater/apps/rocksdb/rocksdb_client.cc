...
/* server-side stat */
typedef RocksDBStatResp sstat_raw;
...
std::vector<work_unit> ClientWorker(
    ...
    auto th = rt::Thread([&] {
            ...
            while (true) {
                ...
                if (ret != static_cast<ssize_t>(sizeof(RocksDBResp))) {
                    if (ret == 0 || ret < 0) break;
                    panic("read failed, ret = %ld", ret);
                }

                // Validate the response
                const RocksDBResp *resp = reinterpret_cast<const RocksDBResp *>(resp_buf);
                if (unlikely(ntoh64(resp->magic) != ROCKSDB_RESP_MAGIC)) {
    ...
    for (unsigned int i = 0; i < wsize; ++i) {
        ...
        RocksDBReq *req = reinterpret_cast<RocksDBReq *>(buf);
        req->magic = hton64(ROCKSDB_REQ_MAGIC);
        req->opaque = hton64(i);
