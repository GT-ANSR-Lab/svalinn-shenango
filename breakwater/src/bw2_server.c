...
static atomic_t srpc_num_sess[NSESSIONTYPE];

/* the number of drained session */
static atomic_t srpc_num_drained[NSESSIONTYPE];

/* the number of active sessions */
static atomic_t srpc_num_active[NSESSIONTYPE];

/* global credit pool */
static atomic_t srpc_credit_pool[NSESSIONTYPE];

/* timestamp of the latest credit pool update */
static uint64_t srpc_last_cp_update;

/* global credit used */
static atomic_t srpc_credit_used[NSESSIONTYPE];

/* downstream credit for multi-hierarchy */
static atomic_t srpc_credit_ds[NSESSIONTYPE];

/* the number of pending requests */
static atomic_t srpc_num_pending[NSESSIONTYPE];

/* EWMA execution time */
static atomic_t srpc_avg_st[NSESSIONTYPE];

static double credit_carry[NSESSIONTYPE];
...
static spinlock_t srpc_cm_lock;
static atomic64_t srpc_cm_in_cnt;
static atomic64_t srpc_cm_out_cnt;
static atomic64_t srpc_cm_drop_cnt;
...
static void srpc_update_credit_pool()
{
    ...
    if (!spin_try_lock_np(&srpc_cm_lock)) {
        return;
    }
