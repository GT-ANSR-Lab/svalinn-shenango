...
#define CPCC_TRACK_FLOW         false
#define CPCC_TRACK_FLOW_ID      1
 ...
 */
static ssize_t crpc_send_cupdate(struct cpcc_conn *cc)
{
    struct cpcc_session *s = cc->session;
    ...
    chdr.magic = PCC_REQ_MAGIC;
    chdr.op = PCC_OP_CREDIT;
    chdr.id = 0;
    chdr.len = 0;
...
#if CPCC_TRACK_FLOW
    if (cc->session->id == CPCC_TRACK_FLOW_ID) {
...
static ssize_t crpc_send_request_vector(struct cpcc_conn *cc)
{
    struct cpcc_session *s = cc->session;
    struct cpcc_hdr chdr[CRPC_QLEN];
    ...
    while (s->head != s->tail && cc->credit_used < cc->credit) {
        ...
        chdr[nrhdr].magic = PCC_REQ_MAGIC;
        chdr[nrhdr].op = PCC_OP_CALL;
        chdr[nrhdr].id = c->id;
        ...
        vec[nriov].iov_len = sizeof(struct cpcc_hdr);
        nrhdr++;
        nriov++;
...
static ssize_t crpc_send_raw(struct cpcc_conn *cc,
...
static void crpc_drain_queue(struct cpcc_session *s)
{
    int pos;
    struct crpc_ctx *c;
    uint64_t now = microtime();
    ...
    while (s->head != s->tail) {
        ...
        c = s->qreq[pos];
        if (CPCC_MAX_CLIENT_DELAY_US == 0 ||
            now - c->ts <= CPCC_MAX_CLIENT_DELAY_US)
            break;
    ...
    for (i = 0; i < s->cmn.nconns; ++i) {
        ...
        cc = (struct cpcc_conn *)s->cmn.c[conn_idx];
...
static bool crpc_enqueue_one(struct cpcc_session *s,
...
int cpcc_add_connection(struct crpc_session *s_, struct netaddr raddr)
{
    struct cpcc_session *s = (struct cpcc_session *)s_;
...
ssize_t cpcc_send_one(struct crpc_session *s_, const void *buf, size_t len,
                      int hash, void *arg)
{
    struct cpcc_session *s = (struct cpcc_session *)s_;
    struct cpcc_conn *cc;
    ...
    /* hot path, just send */
    cc = (struct cpcc_conn *)s->cmn.c[s->next_conn_idx];
...
ssize_t cpcc_recv_one(struct crpc_conn *cc_, void *buf, size_t len, void *arg)
{
    struct cpcc_conn *cc = (struct cpcc_conn *)cc_;
    struct cpcc_session *s = cc->session;
...
again:
    ...
    if (unlikely(shdr.magic != PCC_RESP_MAGIC)) {
...
{
    struct cpcc_session *s = (struct cpcc_session *)arg;
    uint64_t now;
    int pos;
    struct crpc_ctx *c;
    int num_drops;
    ...
    while(true) {
        ...
        while (s->head != s->tail) {
            pos = s->tail % CRPC_QLEN;
            c = s->qreq[pos];
            if (now - c->ts <= CPCC_MAX_CLIENT_DELAY_US)
                break;
        ...
        c = s->qreq[pos];
        mutex_unlock(&s->lock);
        timer_sleep_until(c->ts + CPCC_MAX_CLIENT_DELAY_US);
        mutex_lock(&s->lock);
    }
done:
    mutex_unlock(&s->lock);
...
int cpcc_open(struct netaddr raddr, struct crpc_session **sout, int id,
              crpc_ldrop_fn_t ldrop_handler, crpc_rdrop_fn_t rdrop_handler,
              struct rpc_session_info *info)
{
    struct netaddr laddr;
    struct cpcc_session *s;
    struct cpcc_conn *cc;
    tcpconn_t *c;
    int i, ret;
...
void cpcc_close(struct crpc_session *s_)
{
    struct cpcc_session *s = (struct cpcc_session *)s_;
    int i;
...
/* client-side stats */
uint32_t cpcc_conn_credit(struct crpc_conn *cc_)
{
    struct cpcc_conn *cc = (struct cpcc_conn *)cc_;
    return cc->credit;
}

uint32_t cpcc_sess_credit(struct crpc_session *s_)
{
    struct cpcc_session *s = (struct cpcc_session *)s_;
    uint32_t ret = 0;

    for(int i = 0; i < s->cmn.nconns; ++i) {
        ret += cpcc_conn_credit(s->cmn.c[i]);
    }

    return ret;
}

void cpcc_conn_stat_clear(struct crpc_conn *cc_)
{
    struct cpcc_conn *cc = (struct cpcc_conn *)cc_;
...
void cpcc_sess_stat_clear(struct crpc_session *s_)
{
    struct cpcc_session *s = (struct cpcc_session *)s_;

    s->req_dropped_ = 0;
    for(int i = 0; i < s->cmn.nconns; ++i) {
        cpcc_conn_stat_clear(s->cmn.c[i]);
    }
}

uint64_t cpcc_conn_stat_credit_expired(struct crpc_conn *cc_)
{
    struct cpcc_conn *cc = (struct cpcc_conn *)cc_;
    return cc->credit_expired_;
}

uint64_t cpcc_sess_stat_credit_expired(struct crpc_session *s_)
{
    struct cpcc_session *s = (struct cpcc_session *)s_;
    uint64_t ret = 0;

    for(int i = 0; i < s->cmn.nconns; ++i) {
        ret += cpcc_conn_stat_credit_expired(s->cmn.c[i]);
    }

    return ret;
}

uint64_t cpcc_conn_stat_ecredit_rx(struct crpc_conn *cc_)
{
    struct cpcc_conn *cc = (struct cpcc_conn *)cc_;
    return cc->ecredit_rx_;
}

uint64_t cpcc_sess_stat_ecredit_rx(struct crpc_session *s_)
{
    struct cpcc_session *s = (struct cpcc_session *)s_;
    uint64_t ret = 0;

    for(int i = 0; i < s->cmn.nconns; ++i) {
        ret += cpcc_conn_stat_ecredit_rx(s->cmn.c[i]);
    }

    return ret;
}

uint64_t cpcc_conn_stat_cupdate_tx(struct crpc_conn *cc_)
{
    struct cpcc_conn *cc = (struct cpcc_conn *)cc_;
    return cc->cupdate_tx_;
}

uint64_t cpcc_sess_stat_cupdate_tx(struct crpc_session *s_)
{
    struct cpcc_session *s = (struct cpcc_session *)s_;
    uint64_t ret = 0;

    for(int i = 0; i < s->cmn.nconns; ++i) {
        ret += cpcc_conn_stat_cupdate_tx(s->cmn.c[i]);
    }

    return ret;
}

uint64_t cpcc_conn_stat_resp_rx(struct crpc_conn *cc_)
{
    struct cpcc_conn *cc = (struct cpcc_conn *)cc_;
    return cc->resp_rx_;
}

uint64_t cpcc_sess_stat_resp_rx(struct crpc_session *s_)
{
    struct cpcc_session *s = (struct cpcc_session *)s_;
    uint64_t ret = 0;

    for(int i = 0; i < s->cmn.nconns; ++i) {
        ret += cpcc_conn_stat_resp_rx(s->cmn.c[i]);
    }

    return ret;
}

uint64_t cpcc_conn_stat_req_tx(struct crpc_conn *cc_)
{
    struct cpcc_conn *cc = (struct cpcc_conn *)cc_;
    return cc->req_tx_;
}

uint64_t cpcc_sess_stat_req_tx(struct crpc_session *s_)
{
    struct cpcc_session *s = (struct cpcc_session *)s_;
    uint64_t ret = 0;

    for(int i = 0; i < s->cmn.nconns; ++i) {
        ret += cpcc_conn_stat_req_tx(s->cmn.c[i]);
    }

    return ret;
}

uint64_t cpcc_sess_stat_req_dropped(struct crpc_session *s_)
{
    struct cpcc_session *s = (struct cpcc_session *)s_;
    return s->req_dropped_;
}

struct crpc_ops cpcc_ops = {
    .crpc_add_connection        = cpcc_add_connection,
    .crpc_send_one              = cpcc_send_one,
    .crpc_recv_one              = cpcc_recv_one,
    .crpc_open                  = cpcc_open,
    .crpc_close                 = cpcc_close,
    .crpc_credit                = cpcc_sess_credit,
    .crpc_stat_clear            = cpcc_sess_stat_clear,
    .crpc_stat_ecredit_rx       = cpcc_sess_stat_ecredit_rx,
    .crpc_stat_credit_expired   = cpcc_sess_stat_credit_expired,
    .crpc_stat_cupdate_tx       = cpcc_sess_stat_cupdate_tx,
    .crpc_stat_resp_rx          = cpcc_sess_stat_resp_rx,
    .crpc_stat_req_tx           = cpcc_sess_stat_req_tx,
    .crpc_stat_req_dropped      = cpcc_sess_stat_req_dropped,
};
