...
/**
 * semaphore_init - initializes a semaphore
 * @s: the semaphore to initialize
 * @count: number of threads that can acquire the semaphore
 */
void semaphore_init(semaphore_t *s, int count)
{
    ...
    assert(count > 0);
    s->count = count;
    list_head_init(&s->waiters);
}

/**
 * semaphore_wait - acquire a semaphore
 * @s: the semaphore
 */
...
/**
 * semaphore_try_wait - try to acquire a semaphore
 * @s: the semaphore
 */
bool semaphore_try_wait(semaphore_t *s)
{
    bool acquired = false;
    spin_lock_np(&s->lock);
...
/**
 * semaphore_post - release a semaphore
 * @s: the semaphore
 */
void semaphore_post(semaphore_t *s)
{
    ...
    if (!list_empty(&s->waiters)) {
        waketh = list_pop(&s->waiters, thread_t, link);
        assert(waketh);
        spin_unlock_np(&s->lock);
        thread_ready(waketh);
    } else {
